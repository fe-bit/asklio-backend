...
from .models.db import ProcurementRequest, OrderLine, Status
from .models.dto import ProcurementRequestCreateDTO, ProcurementRequestUpdateDTO
from .db import get_db_connection, serialize_order_lines, deserialize_order_lines, get_commodity_group, get_procurement_request
...
async def create_request(req: ProcurementRequestCreateDTO):
    ...
    proc_request = ProcurementRequest(**req.dict(), commodity_group=commodity_group, id=None)
    c.execute('''INSERT INTO procurement_requests (
        ...
        proc_request.requestor_name, proc_request.title, proc_request.vendor_name, proc_request.vat_id, commodity_group.id,
        proc_request.total_cost, proc_request.department, proc_request.status, serialize_order_lines(proc_request.order_lines)
    ))
    conn.commit()
    new_id = c.lastrowid
    return ProcurementRequest(id=new_id, **{**proc_request.dict(exclude={'id', 'commodity_group'}), 'commodity_group': commodity_group})
...
async def list_requests():
    ...
    for row in rows:
        ...
        result.append(ProcurementRequest(
            ...
            commodity_group=cg, total_cost=row[6], department=row[7], status=row[8],
...
async def update_request(request_id: int, req: ProcurementRequestUpdateDTO):
    ...
    c.execute('''UPDATE procurement_requests SET
        ...
        WHERE id=?''', (
        req.requestor_name, req.title, req.vendor_name, req.vat_id,
